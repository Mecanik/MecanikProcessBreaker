...
namespace MecanikDetours
{
	...
	class MNTDetours
	{
		public:
			...
			static VOID WINAPI NTHook();

			DWORD RIPFunction(LPCSTR lpModule, LPCSTR lpFuncName, LPVOID lpFunction, unsigned char *lpBackup);
			BOOL UNRIPFunction(LPCSTR lpModule, LPCSTR lpFuncName, unsigned char *lpBackup);

			void CheckIfPatched(const char* szDll, const char* szFunc);
			void CheckForIATHook(const char* szDll, const char* szFunc);

		private:
			void GetSystemMessage(char* Function);
	};
	extern MNTDetours T_MNTDetours;
}